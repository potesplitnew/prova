...
// Ogni "persona" di un evento è ora uno username che fa riferimento a un
// account del registro centrale (vedi js/users.js): ND() risolve lo username
// nel nome visualizzato dell'account e lo rende sicuro per l'HTML, da usare
// ovunque un nome venga MOSTRATO. Per confronti/identificatori si continua a
// usare direttamente lo username (invariato: === , .includes, chiavi di oggetti…).
function ND(username) {
  return escapeHtml(Utenti.nome(username));
}
...
async function loadAllData() {
  ...
  const [persone, spese, rimborsi, cene, config] = await Promise.all([
    ...
    Utenti.carica() // registro account: serve per risolvere username -> nome visualizzato
  ]);
...
function formatPartecipanti(partecipanti) {
  ...
  return [...partecipanti].map(p => Utenti.nome(p)).sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())).join(", ");
}
...
function renderRegistroSpese() {
  ...
  ordinate.forEach((s, i) => {
    ...
    const tr = el("tr", null, `<td>${i + 1}</td><td>${ND(s.nome)}</td><td>${escapeHtml(s.descrizione)}</td><td class="num">${euro(s.importo)}</td><td>${escapeHtml(part)}</td>`);
...
function renderRimborsiEffettuati() {
  ...
  STATE.rimborsi.forEach((r, i) => {
    tbody.appendChild(el("tr", null, `<td>${i + 1}</td><td>${ND(r.da)}</td><td>${ND(r.a)}</td><td class="num">${euro(r.importo)}</td>`));
  });
}
...
function renderTotaliPersona() {
  ...
  nomi.forEach((nome, i) => {
    ...
    const tr = el("tr", cls, `<td>${i + 1}</td><td>${ND(nome)}</td><td class="num">${euro(totaliPersona[nome])}</td>
...
function renderTransazioni() {
  ...
  trans.forEach((t, i) => {
    ...
    tbody.appendChild(el("tr", cls, `<td>${i + 1}.</td><td>${ND(t.da)}</td><td>${ND(t.a)}</td><td class="num">${euro(t.importo)}</td>`));
  });
}
...
function renderCredenziali() {
  ...
  elenco.forEach(c => {
    ...
    box.appendChild(el("li", null, `<strong>${ND(c.nome)}</strong> — ${dettaglio}`));
  });
}
...
function renderCategoriaTabella(container, cena, categorie, header) {
  ...
  cena.persone.forEach(p => {
    ...
    html += `<tr><td>${ND(p.nome)}</td>${celle.join("")}<td class="tot-cell">${totStr}</td></tr>`;
  });
...
function renderCondivise(container, cena) {
  ...
  cena.persone.forEach(p => {
    ...
    html += `<tr><td>${ND(p.nome)}</td>${cells.join("")}</tr>`;
  });
  html += "</tbody></table></div>";
  container.innerHTML = html;
}
...
function renderTotaliECena(container, cena) {
  ...
  d.righe.forEach(r => {
    ...
    html += `<tr class="${cls}"><td>${ND(r.nome)}</td><td class="num">${euro(r.dovuto)}</td><td class="num">${euro(r.pagato)}</td><td class="num">${euro(r.saldo)}</td></tr>`;
  });
  if (d.hasSconti) {
  ...
  if (d.transazioniCena.length === 0) {
    ...
    d.transazioniCena.forEach(t => html += `<tr><td>${ND(t.da)}</td><td>${ND(t.a)}</td><td class="num">${euro(t.importo)}</td></tr>`);
    html += "</tbody></table></div>";
  }
  container.innerHTML = html;
}
...
function renderRiepilogoGruppoSpesa(container, g) {
  ...
  r.partecipanti.forEach(nome => {
    ...
    html += `<tr class="${cls}"><td>${ND(nome)}</td>
  ...
  if (r.transazioni.length === 0) {
    ...
    html += `<div class="table-wrap"><table class="cena-table"><thead><tr><th>Da</th><th>A</th><th>Importo</th></tr></thead><tbody>`;
    r.transazioni.forEach(t => html += `<tr><td>${ND(t.da)}</td><td>${ND(t.a)}</td><td class="num">${euro(t.importo)}</td></tr>`);
    html += "</tbody></table></div>";
  }
  container.innerHTML = html;
}
...
function renderAll() {
  ...
  renderListaPersone();
  renderCheckboxAccountDisponibili();
  renderListaSpese();
  renderListaRimborsi();
  renderListaCene();
...
function populateFormSelects() {
  const persone = [...STATE.persone].sort((a, b) => Utenti.nome(a).toLowerCase().localeCompare(Utenti.nome(b).toLowerCase()));
  const selects = document.querySelectorAll("select.persona-select");
  selects.forEach(sel => {
    const current = sel.value;
    sel.innerHTML = persone.map(p => `<option value="${escapeHtml(p)}">${ND(p)}</option>`).join("");
  ...
  checkboxContainers.forEach(box => {
    ...
    box.innerHTML = persone.map(p =>
      `<label class="chk"><input type="checkbox" value="${escapeHtml(p)}" ${checked.has(p) ? "checked" : ""}> ${ND(p)}</label>`).join("");
  });
}
...
// Le persone di un evento sono ora account del registro centrale (vedi js/users.js),
// non più testo libero: non ha più senso "rinominare" una persona qui (si
// rinominerebbe l'account, cosa che spetta solo all'amministratore dalla home).
// Chi organizza può solo rimuovere una persona dall'evento (a cascata, come prima)
// e ri-aggiungerla eventualmente in seguito dal riquadro "Aggiungi persona".
function renderListaPersone() {
  ...
  [...STATE.persone].sort((a, b) => Utenti.nome(a).toLowerCase().localeCompare(Utenti.nome(b).toLowerCase())).forEach(nome => {
    const row = el("div", "list-row");
    row.innerHTML = `
      <div class="list-main">${ND(nome)}<div class="list-sub">account: ${escapeHtml(nome)}</div></div>
      <div class="list-actions">
        <button class="btn-icon delete" type="button">Rimuovi dall'evento</button>
      </div>`;
...
function renderListaSpese() {
  ...
  gruppi.forEach(g => {
    const chiPaga = g.voci.map(s => `${ND(s.nome)} (${euro(s.importo)})`).join(", ");
...
function renderListaRimborsi() {
  ...
  STATE.rimborsi.forEach((r, i) => {
    ...
    row.innerHTML = `
      <div class="list-main">${ND(r.da)} → ${ND(r.a)}: <strong>${euro(r.importo)}</strong></div>
      <div class="list-actions">
        <button class="btn-icon edit" type="button">Modifica</button>
        <button class="btn-icon delete" type="button">Elimina</button>
      </div>`;
    ...
    row.querySelector(".delete").addEventListener("click", async () => {
      if (!confirm(`Eliminare il rimborso ${Utenti.nome(r.da)} → ${Utenti.nome(r.a)} (${euro(r.importo)})?`)) return;
      try {
...
// --- Aggiungi persona/e ---
// Le persone si scelgono ora tra gli ACCOUNT già registrati centralmente (dalla home,
// solo l'amministratore può crearne di nuovi): niente più testo libero. Qui si mostra
// la lista degli account non ancora partecipanti a questo evento, con una checkbox
// ciascuno; se un account non esiste ancora, va prima creato dall'amministratore
// nella sezione "Gestione account" della home.
function renderCheckboxAccountDisponibili() {
  const box = document.querySelector("#f-persona-account-container");
  const disponibili = Utenti.disponibili(STATE.persone)
    .sort((a, b) => a.nome.toLowerCase().localeCompare(b.nome.toLowerCase()));
  if (disponibili.length === 0) {
    box.innerHTML = `<div class="empty-note">Tutti gli account registrati partecipano già a questo evento, oppure non è ancora stato creato nessun account: chiedi all'amministratore di crearne uno dalla home.</div>`;
    return;
  }
  box.innerHTML = disponibili.map(u =>
    `<label class="chk"><input type="checkbox" class="f-persona-account-check" value="${escapeHtml(u.username)}"> ${escapeHtml(u.nome)} <span class="list-sub">(${escapeHtml(u.username)})</span></label>`
  ).join("");
}

async function submitPersona(e) {
  e.preventDefault();
  const selezionati = Array.from(document.querySelectorAll("#f-persona-account-container .f-persona-account-check:checked")).map(i => i.value);
  if (selezionati.length === 0) { setStatus("#status-persona", "Seleziona almeno un account.", true); return; }

  try {
    const nuovoElenco = [...STATE.persone, ...selezionati];
    const nomiLeggibili = selezionati.map(u => Utenti.nome(u));
    const msg = selezionati.length > 1 ? `Aggiunge ${selezionati.length} persone: ${nomiLeggibili.join(", ")}` : `Aggiunge persona: ${nomiLeggibili[0]}`;
    await GH.writeJSON("data/persone.json", nuovoElenco, msg);
    setStatus("#status-persona", (selezionati.length > 1 ? "Persone aggiunte!" : "Persona aggiunta!") + " Ricarico i dati…", false);
    await loadAllData(); renderAll();
    renderCheckboxAccountDisponibili();
...
function renderSpesaQuoteRighe() {
  ...
  checked.forEach(nome => {
    ...
    row.innerHTML = `
      <div class="cp-persona-titolo" style="margin:0;">${ND(nome)}</div>
...
function buildCenaPersoneRow(nome, dati) {
  ...
  row.dataset.nome = nome;
  row.innerHTML = `
    <div class="cp-persona-titolo">${ND(nome)}</div>
...
function refreshCenaPersoneCheckboxes() {
  ...
  box.innerHTML = ordinati.map(p => {
    ...
    return `<label class="chk"><input type="checkbox" value="${escapeHtml(p)}" ${spuntata ? "checked" : ""}> ${ND(p)}</label>`;
  }).join("");
...
async function submitCena(e) {
  ...
  for (const row of Array.from(document.querySelectorAll("#cena-persone-rows .cena-persona-row"))) {
    ...
    for (const c of CAT_INPUT) {
      ...
      if (v < 0) { setStatus("#status-cena", `Il valore di "${CAT_LABELS[c]}" per ${Utenti.nome(p.nome)} non può essere negativo.`, true); return; }
      p[c] = v;
    }
    persone.push(p);
  }
...
function renderListaCredenziali() {
  ...
  elenco.forEach((c, i) => {
    ...
    row.innerHTML = `
      <div class="list-main">${ND(c.nome)}<br><span class="list-sub">${dettaglio}</span></div>
      <div class="list-actions">
        <button class="btn-icon edit" type="button">Modifica</button>
        <button class="btn-icon delete" type="button">Elimina</button>
      </div>`;
    ...
    row.querySelector(".delete").addEventListener("click", async () => {
      if (!confirm(`Eliminare le credenziali di "${Utenti.nome(c.nome)}"?`)) return;
      try {
